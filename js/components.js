...
$(document).ready(function(){
	$('.button--more').click(function(){
		$('.calender-content').toggleClass('show-calender-content');
		$(this).toggleClass('rotate-icon');
	});


	var linkCollapse = $('a[data-toggle="collapse"]'),
	tabCollapse = $('.panel-collapse.collapse');
	for(var i = 0; i < linkCollapse.length; i++){
		$(linkCollapse[i]).attr('href','#collapse' + i);
		$(tabCollapse[i]).attr('id','collapse' + i)
	}
});
...
var $modalWindow = jQuery('.bs-example-modal-md.main-modal');
var $trigger = jQuery('.open-form');
var $closeTriggers = jQuery('#overlay, .close_form');
...
function onSignIn(googleUser) {
	...
	jQuery("input[data-validation-type='lastname']").val(name);
	jQuery("input[data-validation-type='email']").val(email);
	console.log(email + ":" + name);
	if (name) {
		var $numberTitle = jQuery('.form-title_number');
		var $excessFields = {
			name: jQuery('.form_group--name'),
			email: jQuery('.form_group--email'),
			title: jQuery('.modal--title')
		};
		$trigger.trigger('click');
		for (var el in $excessFields) {
			$excessFields[el].css({display: "none"});
		}
		$numberTitle.show();
	}
	signOut();
...
jQuery('.tabs_list--item .title').on('click', function () {
	jQuery(this).toggleClass('active');
});
var $events_list = jQuery('.events_list');

jQuery('.more span').on('click', function () {
	jQuery(this).toggleClass('active');
	$events_list.toggleClass('active');
});
	...
	if (Ra.bottom < R1bottom) {
      ...
      if (Ra.top < K) {  // скролл вниз
        if (R2 + N > R1) {  // не дойти до низа
          ...
          if (Rb.bottom - W + N <= 25) {  // подцепиться
          	b.className = 'sticky';
...
//     jQuery('.post_page').addClass("hidden").viewportChecker({
